...
- `InfinitePhaseScreen` now shares its A/B extrusion matrices between instances with the same geometry and turbulence parameters (they do not depend on the random seed), through a weak-valued cache that drops an entry once no screen uses it. Results are bitwise identical; a repeated construction drops from ~1 s to ~20 ms (512 pixels), e.g. test\_infinite\_vs\_fft\_phase\_screen\_statistics goes from 43 s to 5 s.
